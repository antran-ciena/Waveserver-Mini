...
void recalculate_oper_state(port_t *port)
{
    ...
    port->operational_state =
        (port->admin_enabled && !port->fault_active) ? PORT_UP : PORT_DOWN;
        ...
        if (now - last_health_check >= 5)
        {
            ...
            for (int i = 0; i < MAX_PORT_NUM; i++)
            {
                ...
                LOG(LOG_INFO,
                    ...
                    i,
                    (p->type == LINE_PORT) ? "LINE" : "CLIENT",
                    p->admin_enabled ? "Enabled" : "Disabled",
                    p->fault_active ? "Active" : "None",
                    (p->operational_state == PORT_UP) ? "UP" : "DOWN",
                    p->rx_frames,
                    p->dropped_frames);
            }
        }
