...
#define PROTECTION_MGR_UDP (5004)   /* NEW — protection manager */
...
typedef enum
{
    ...
    MSG_PORT_STATE_CHANGE, // Port Mgr → Conn Mgr + Prot Mgr : port_state_change_t → (no reply)
    ...
    // Protection Manager messages (NEW)
    MSG_SET_PROTECTION,    // CLI → Prot Mgr : activate protection group (request → reply)
    MSG_DELETE_PROTECTION, // CLI → Prot Mgr : deactivate protection group (request → reply)
    MSG_GET_PROTECTION,    // CLI → Prot Mgr : show protection group state (request → reply)
 
    // Connection Manager — used internally by Protection Manager (NEW)
    MSG_SWITCH_CONN_LINE,  // Prot Mgr → Conn Mgr : move connection to new line port (request → reply)
} msg_type_t;
...
// MSG_SWITCH_CONN_LINE request — Prot Mgr → Conn Mgr (NEW)
typedef struct
{
    char    conn_name[MAX_CONN_NAME_CHARACTER];
...
// One row in the MSG_GET_PROTECTION reply (NEW)
typedef struct
{
    char    conn_name[MAX_CONN_NAME_CHARACTER];
    uint8_t client_port;
...
// MSG_GET_PROTECTION reply — Prot Mgr → CLI (NEW)
typedef struct
{
    bool             active;
    uint32_t         switchover_count;
    uint8_t          entry_count;
    udp_prot_entry_t entries[MAX_CONNS];
} udp_get_protection_reply_t;
