...
{
    const udp_create_conn_request_t *payload =
        (const udp_create_conn_request_t *)req->payload;

    resp->status = STATUS_FAILURE;

    if (strlen(payload->name) == 0 || strlen(payload->name) >= MAX_CONN_NAME_CHARACTER)
    {
    ...
    {
        set_error_msg(resp, "could not get line port info");
        return;
    }

    if (!get_port_info(payload->client_port, &client_port_info))
    {
        set_error_msg(resp, "could not get client port info");
        return;
    }

    if (line_port_info.operational_state != PORT_UP ||
        client_port_info.operational_state != PORT_UP)
    {
    ...
    LOG(LOG_INFO, "created connection '%s': client-%d -> line-%d state=%s",
        slot->conn_name,
        slot->client_port,
        slot->line_port,
        slot->operational_state == CONN_UP ? "UP" : "DOWN");
}
...
{
    const udp_delete_conn_request_t *udp_payload =
        (const udp_delete_conn_request_t *)req->payload;

    conn_t *found_conn = find_connection_by_name(udp_payload->name);
    if (found_conn == NULL)
    {
        set_error_msg(resp, "could not find connection with that name");
        LOG(LOG_ERROR, "could not find connection with that name (name=%s)",
            udp_payload->name);
        return;
    }
